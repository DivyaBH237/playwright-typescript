...
import signInData from "../testdata/signInData.json";
...
test.describe("SignIn Page validations Tests", () => {
  ...
  test("[AQAPRACT-535] Sign in with invalid email and valid password", async ({
    ...
    await signInPage.fillEmail(signInData.invalidEmail);
    await signInPage.fillPassword(signInData.validPassword);
    await signInPage.clickSignIn();
    await expect(signInPage.signInButton).toBeDisabled();
    await signInPage.expectPasswordFieldErrorBorder(signInPage.emailField);
  ...
  test("[AQAPRACT-536] Sign in with valid email and invalid password", async ({
    ...
    await signInPage.fillEmail(signInData.validEmail);
    await signInPage.fillPassword(signInData.invalidPassword);
    await signInPage.clickSignIn();
    await expect(signInPage.signInButton).toBeDisabled();
    await signInPage.expectPasswordFieldErrorBorder(signInPage.passwordField);
    await signInPage.expectInvalidLoginError();
  });

  test("[AQAPRACT-537] Sign in with invalid email and invalid password", async ({
    signInPage,
  }) => {
    await signInPage.fillEmail(signInData.invalidEmail);
    await signInPage.fillPassword(signInData.invalidPassword);
  ...
  test("[AQAPRACT-538] Sign in with email address with invalid format", async ({
    ...
    await signInPage.fillEmail(signInData.invalidEmailFormat);
    await signInPage.fillPassword(signInData.validPassword);
...
test.describe("SignIn Page Email Field Validation Test", () => {
  test('[AQAPRACT-539] Validation of empty "Email" field on "Sign in" page', async ({
    ...
    await signInPage.fillEmail(signInData.emptyEmail);
    await signInPage.fillPassword(signInData.validPassword);
...
test.describe("SignIn Page Password Field Validation", () => {
  ...
  test('[AQAPRACT-543] Validation of "Password" on 7 characters', async ({
    ...
    await signInPage.fillEmail(signInData.validEmail);
    await signInPage.fillPassword(signInData.shortPassword);
    await signInPage.validatePasswordError("Minimum 8 characters");
    await signInPage.expectPasswordFieldErrorBorder(signInPage.passwordField);
    await expect(signInPage.signInButton).toBeDisabled();
  });

  test('[AQAPRACT-544] Validation of "Password" on 20 characters', async ({
    signInPage,
  }) => {
    await signInPage.fillEmail(signInData.validEmail);
    await signInPage.fillPassword(signInData.longPassword);
